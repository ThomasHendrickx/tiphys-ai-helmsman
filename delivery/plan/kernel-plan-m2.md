...
3. **RESOLVED as DR-0014, decided 2026-08-05: release verification is a PLUGGABLE INTERFACE, not a platform selector.** This item asked which platform the deploy verifier's first adapter targets. The owner rejected the framing: one instance may need a Vercel check, another Hetzner, another a local project with no remote release, another a mobile app. The kernel owns the contract and ships reference adapters. The number was assigned at reconciliation as foreseen here, and DR-0013 went to the validator question as the M3 plan had claimed it. M2-P7 step 2 makes this consequential rather than cosmetic: with a reachable platform the poller ships with captured-response semantics; without one it does not ship at all and moves to M4. Blueprint owner action 2 (pick the pilot) is what would answer it.
