...
1. **`tiphys spawn` ships no agent adapter**, which stands. The SECOND half of
   this defect as reported, that a scrubbed child cannot authenticate, **IS NOT
   REPRODUCIBLE IN THIS CONTAINER AND WAS MEASURED FALSE 2026-09-15.** A Claude
   Code payload authenticates under the kernel's real seven-variable scrubbed
   environment, and under PATH alone, because the credential sits at an absolute
   path outside `$HOME` where redirecting `HOME` cannot reach it. What the scrub
   does break is `git push`, at a cost of exactly one name. Full account and the
   honest limit (the prober could not distinguish "the pilot was wrong" from
   "the pilot ran somewhere else") at
...
| M4-D-07 | **How does a real agent payload authenticate under the credential scrub, and may the plugin set `allowPrCredentials`?** | **THE PROBE RAN AND THE FIRST HALF OF THE QUESTION DISSOLVED.** Measured: it authenticates already; the minimal `extraAllowlist` for model authentication is the EMPTY LIST. `git push` is what breaks, and it costs exactly `HTTPS_PROXY`; `GH_TOKEN` carries nothing. **Two findings replace the original question**: the `credential-scrub` gate's verdict is INVERTED with respect to real capability here (it greens the name that grants push), and the real boundary is the UID and the FILESYSTEM, which the kernel's environment-only scrub does not touch since it spawns at the same uid. The `allowPrCredentials` half stands and the recommendation below is unchanged. See delivery/verification/m4-prototype-probes.md:1. Original framing kept below. ORCHESTRATOR, WAS PROTOTYPE-BLOCKED. Recommend: `extraAllowlist` wired through `spawnTask` (it exists as data at src/exec/env.ts:154 and nothing reaches it), with a WRITTEN per-name reason for every crossing name; `allowPrCredentials` FORBIDDEN for any project payload, as a declared constraint. The red witness must be observed FROM INSIDE the agent turn, because the gate probes the constructed environment and a guard that checks only the object is the cannot-go-red shape this repository keeps paying for (H13, H31, H67) |
...
| M4-D-11 | **Does the distributed lease live in the tracked fleet tree, outside git, or on a dedicated git ref?** | **CLOSED 2026-09-15: a dedicated BRANCH, and the probe raced two clones to prove it.** Three conditions the recommendation omitted: only `refs/heads/*` is pushable here; the expectation must be an explicit sha, because the bare `--force-with-lease` form is disarmed by any fetch and silently clobbered a live holder; and nonzero does not mean "I lost", so the API needs three states. Stronger than claimed: safety does not depend on the freshness rule being right. See delivery/verification/m4-prototype-probes.md:1. ORCHESTRATOR, WAS PROTOTYPE-BLOCKED. Recommend a dedicated ref used as a compare-and-swap register through `git push --force-with-lease`, a shape neither the plan nor DR-0007 considers. It needs no new service, it is a genuine CAS, and its safety does not depend on comparing two machines' wall clocks, which matters because the tolerance today is five seconds and C-2 forbids the obvious escape. The probe is whether the remote honours `--force-with-lease` atomically |
