...
| c `retirement` | `tiphys cutover status --retirement` reports zero `unported` rows | any `unported` row is `not-yet`; ZERO ROWS ALTOGETHER is `unreachable`, and so is a line carrying `ported` that is not a `PORT <name> ported` or `PORT <name> unported` row, or an exit 0 printed alongside an unported row |
