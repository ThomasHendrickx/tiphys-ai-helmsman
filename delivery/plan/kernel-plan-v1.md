...
- Baseline commit: ec3765fde34ce8197c86449b7eefb2d177598ed6
...
- steps:
  1. Create src/lock.ts: lease-based session lock (decided DR-0007; plan constraint C-2: no pid-based identity anywhere). The lockfile at state/orchestrator.lock contains JSON {holderId, hostname, acquiredAt, expiresAt}, where holderId is an opaque token generated at acquire, never a pid. Acquisition is atomic (PR-006): open with O_EXCL; on EEXIST, read the existing lease; an unexpired lease refuses acquire, an expired lease also refuses but reports expired (takeover is always explicit). The default lease duration is 900 seconds, configurable; the renewal rule is that the holder renews at or before half-life (PR-203). lock renew extends expiresAt by the lease duration and fails when the caller's holderId does not match the file. Renew and takeover are verify-after-write (PR-202, applying the harvested FM-022 protocol): after the rewrite or rename, re-read the file; if it does not contain the caller's holderId and expected expiresAt, the operation was lost to a concurrent writer and exits nonzero without retry. Takeover (--take-over, valid only on an expired lease) is atomic replace: write a temp file and rename it over the lockfile; takeover of an unexpired lease is refused. status always exits 0 and reports free, held, or expired, printing holderId, acquiredAt, and expiresAt so a human takeover decision is informed. Liveness is lease freshness only; the module contains no process checks (FM-053: firstmate's pid identity accumulated four documented workarounds; lease semantics eliminate the class). Exclusion domain (PR-201): the lease excludes within one filesystem and one clock, the fleet home the lockfile lives in; cross-environment exclusion for a fleet shared through a git remote is part of the M4 residue named in section 3's obligation split and is not claimed by M1. Holdership on mutating commands is verified in M1-P4 (spawn and teardown check the caller's identity against the lease, PR-203), which closes the paused-holder dual-writer window for kernel commands; module docs note that mutations outside the CLI are not covered by that check. Harvested protocol, cited for the brief (FM-017 to FM-022): the acquire-refuse-then-read-only flow and write-then-read-back verification from bin/fm-lock.sh:47-85 and bin/fm-session-lock-lib.sh in the scouted clone; the mechanism is deliberately not ported (FM-058: firstmate's hand-rolled mutex accreted steal protocols; O_EXCL plus atomic rename with the concurrency criterion below is the simpler correct shape, and recovery cleverness must not be added to it).
...
- acceptance criteria:
  ...
  6. Two concurrent lock acquire --take-over invocations on an expired lease yield exactly one exit 0 and one nonzero exit, and the file afterward contains the winner's holderId (PR-202: rename is atomic but not exclusive; the verify-after-write read-back is what the loser fails).
  7. A lock renew raced against a takeover (scripted interleave with a short lease at the expiry boundary) never yields a state where both callers subsequently read their own holderId as the current holder: afterward the file contains exactly one of the two holderIds and the losing operation exited nonzero (PR-202).
  8. grep over src/lock.ts shows no process.kill, no signal-0 probing, no /proc access, and no pid field (C-2, structural inspection).
  9. pool create --task t-a against a scratch project repo exits 0; the worktree at worktrees/t-a has empty git status --porcelain output and its HEAD sha equals the scratch repo default branch head sha.
  10. pool create with an already-used task id exits nonzero and stderr names the id.
  11. Two pool create invocations for distinct task ids launched concurrently both exit 0 and git worktree list in the project shows both worktrees.
  12. pool destroy on a worktree with an uncommitted file exits nonzero and the directory still exists; the same worktree with --discard exits 0, the directory is gone, and git worktree list no longer shows it (PR-010); a clean worktree is likewise removed by pool destroy without flags.
  13. node --test exits 0, total test count strictly greater than M1-P2's count, 0 failing.
...
- acceptance criteria:
  ...
  14. node --test exits 0, total test count strictly greater than M1-P3's count, 0 failing.
...
- acceptance criteria:
  ...
  15. node --test exits 0, total test count strictly greater than M1-P4's count, 0 failing.
...
3. tiphys lock acquire exits 0; the harness records the configured lease duration in the evidence (default 900 seconds), exports TIPHYS_HOLDER_ID from the acquire output for the mutating steps, and renews the lease before step 9's human-bounded wait, recording the renewal (PR-203).
