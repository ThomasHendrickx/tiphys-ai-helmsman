...
- Baseline commit: 50a9368aac6ed25af72d4b4106cd5a2780fb8668
...
- steps:
  ...
  2. Create package.json: name @tiphys/kernel (per DR-0008's recommendation; DR-0008 is deferred, so this is a working assumption due for decision before the M3 plan is approved, a one-line change until first publish), version 0.0.0, private for now, type module, license "Apache-2.0" (DR-0001), engines.node ">=26" (DR-0002), bin mapping tiphys to dist/bin/tiphys.js, a files entry including dist (the publish tarball ships compiled output), scripts: build runs tsc -p tsconfig.json (full type check plus emit into dist/), test runs node --test over test/**/*.test.ts (executed natively via Node 26 type stripping, D-18), prepack runs the build so any publish is freshly built (D-17); devDependencies: typescript at a pinned exact version (PR-003). Generate package-lock.json with npm install and commit it (PR-003).
  3. Create tsconfig.json: strict true, NodeNext module and resolution, target compatible with Node 26, erasableSyntaxOnly true and rewriteRelativeImportExtensions true (relative imports are written with .ts extensions so Node runs the sources natively and tsc rewrites them to .js in emitted output, D-18), outDir dist. Compilation type-checks bin/, src/, and test/, while only bin/ and src/ output is emitted into dist/ (the mechanism, for example a solution-style config with a noEmit test project, is the implementer's choice within acceptance criteria 1 and 9).
  4. Create bin/tiphys.ts: executable dispatcher; subcommand version prints the package.json version and exits 0; any unknown subcommand prints usage to stderr and exits 64; subcommands added by later phases register in one table in src/cli.ts.
...
- files-to-touch (all create; verify absent first): package.json, package-lock.json, tsconfig.json, LICENSE, bin/tiphys.ts, src/cli.ts, src/version.ts, test/cli.test.ts, schemas/README.md, roles/README.md, tuition/README.md, .github/workflows/gates.yml, .gitignore, CLAUDE.md.
- acceptance criteria:
  1. npm ci then npm run build exits 0 (tsc type-checks bin/, src/, and test/ and emits dist/); npm test exits 0 without requiring a prior build (tests run from TypeScript sources via Node 26 type stripping, D-18), and node --test reports N tests with N >= 2 and 0 failing.
  ...
  9. tsconfig.json sets erasableSyntaxOnly to true and rewriteRelativeImportExtensions to true (inspection; these guard the native-run plus compile combination of D-18).
