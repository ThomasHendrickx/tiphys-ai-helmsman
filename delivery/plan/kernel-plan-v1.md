...
3. delivery/decisions/DR-0001 to DR-0004 (license, node floor, CI runner, elevated permissions), all open.
...
| SC-012 | Follows report (escalate-to-owner) | DR-0008 (release registry choice). Does not block M1; blocks the M3 release phase. The plan's M3 outline reads "release v0.1.0 to the registry decided in DR-0008". |
...
M1 content is fixed by blueprint section 13: kernel repo scaffold (package.json, bin/, schemas/, roles/), fleet-home init plus doctor CLI, session lock, worktree pool, spawn, teardown guard, watcher plus liveness guard, toy sandbox project. Six sequential phases. All phases assume the DR-0005 recommendation (Node.js ESM JavaScript, JSDoc types checked by tsc --noEmit --checkJs, node --test, no build step); a different owner choice on DR-0005 triggers a plan revision before dispatch (plan decision D-16). The six firstmate BORROW components are planned as BUILD from blueprint section 4 one-line contracts (plan decision D-1, DR-0009).

Shared phase fields, stated once: migrations: none, this is a library (applies to every M1 phase). parallelizable: no (applies to every M1 phase; M1 policy, section 1.4).
...
- conflicts-with: M1-P2 through M1-P6 (all later phases extend src/cli.js and test/); sequential ordering absorbs this.
- blocked-by: DR-0001, DR-0002, DR-0003, DR-0005. (DR-0004 items 2 and 3 are executed by the owner right after this phase merges; they gate the following merge, not this dispatch.)
...
- conflicts-with: M1-P3 through M1-P6 (src/cli.js, src/fleet.js).
- blocked-by: DR-0005, DR-0007.
...
5. M2-P5 gate runner with preconditions: a machine-readable gate manifest for the kernel repo where each gate declares a precondition (for example "project declares a deploy target in its charter"); the runner reports green, red, or not-applicable per gate and never reports green for an unmet precondition (SC-011). This manifest is the seed the M3 canonical gate registry promotes; kernel-generic gates (typecheck, suite wrapper, scope, citations, coverage) are separated from project-specific gates (deploy, migrations, i18n, analytics), which stay declared-but-not-applicable on the kernel (scout observation 2).
...
- D-16: M1 phases assume the recommendations of DR-0005 (language) and DR-0007 (substrate); a different owner choice on either triggers a plan revision before the affected phases dispatch. Likewise M1-P1 assumes DR-0001/0002/0003 decided values only in fields that are one-line changes if the owner picks another option.
...
| DR | Question | Blocks |
|---|---|---|
| DR-0001 (open) | Repository license | M1-P1 (license field) |
| DR-0002 (open) | Node version floor | M1-P1 (engines, CI matrix) |
| DR-0003 (open) | CI runner | M1-P1 (workflow) |
...
| DR-0005 (open, new) | Kernel language and runtime (SC-004) | Every M1 phase; all of M2 |
...
Per SC-009's disposition (plan decision D-7), every open question is a decision record with status open, not a free-floating list item. The open questions of this plan are exactly: DR-0001, DR-0002, DR-0003, DR-0004, DR-0005, DR-0006, DR-0007, DR-0008, DR-0009. There are no others.
