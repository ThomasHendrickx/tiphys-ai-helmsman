...
| R-041 | S2e, 113 | Gate: typecheck, zero errors | L1 | a gate CLASS the kernel declares, and the project's own registry entry naming its command | M4 | Re-dispositioned by M4-P13. The subject exists in the only M4 subject DR-0037 leaves: package.json's build script, tsc -b, which CLAUDE.md calls the type gate. Retargeted under DR-0028: the kernel ships the class, the kernel's OWN registry declares the command, which is DR-0029's dogfooding sentence |
| R-042 | S2e, 113 | Gate: lint, zero errors, no suppressions | L1 | the project declares the predicate; Tiphys runs it and records the verdict | DR-0029 | Re-dispositioned by M4-P13. No linter and no lint script in the only M4 subject. What lint, format and style mean is the project's, per DR-0029. No-new-suppressions stays computable via diff grep, by the project |
...
| R-045 | S2e, 115 | Gate: locale/i18n parity across all languages | L1 | the project declares the predicate; Tiphys runs it and records the verdict | DR-0029 | Re-dispositioned by M4-P13. Zero i18n files in the only M4 subject, and under DR-0028 the kernel ships no i18n command at any milestone |
| R-046 | S2e, 115-116 | Gate: analytics/telemetry doc kept symmetric with the code, additions AND removals | L1 | the project declares the predicate; Tiphys runs it and records the verdict | DR-0029 | Re-dispositioned by M4-P13, superseding the L1 half of plan decision D-12. Zero analytics files in the only M4 subject. Name symmetry stays computable, by the project; D-12's residue, semantic accuracy via the R-059 blast-radius probe, is untouched |
| R-047 | S2e, 117 | Gate: generated manifests regenerated, drift committed | L1 | a gate CLASS the kernel declares, and the project's own registry entry naming its command | M4 | Re-dispositioned by M4-P13. The subject exists and is already green in the kernel: agent-rules-drift and brief-drift are regenerate-and-compare checks. What is left in M4 is the class declaration plus a red witness |
...
| R-050a | S2e, 121 | Gate: all e2e green | L1 | the project declares the predicate; Tiphys runs it and records the verdict | DR-0029 | Re-dispositioned by M4-P13. Zero e2e files in the only M4 subject; the same reasoning as R-045 |
...
| R-051 | S2e, 123 | Gate: help/docs grep for touched copy | L1 | the project declares the predicate; Tiphys runs it and records the verdict | DR-0029 | Re-dispositioned by M4-P13, and the counter-case is recorded rather than suppressed: the kernel's usage: strings ARE a copy surface and a gate could be built for them. Nothing in M4 consumes one, and building one is the kernel inventing a predicate DR-0029 assigns to the project |
...
| ID | Source | Rule | Target layer | Target form | Milestone | Notes |
|---|---|---|---|---|---|---|
| R-064 | S4, 158 | Merge on CI green only | L1 | branch protection required checks | M4 | Stays M4, note rewritten by M4-P13: already discharged on the kernel repository, whose ruleset main-protection is enforcement active with required_status_checks naming gates. M4 records the evidence. Merge authority per assurance mode is L4 (v1 section 8) |
| R-065a | S4, 158 | Squash merge | L1 | repo merge config (squash-only) | M4 | Stays M4, note added by M4-P13: owner-deferred 2026-09-15 and carried as an M4 exit-criteria footnote. A deferral inside M4 is not a re-disposition, so the bucket does not move |
...
| R-071 | S4, 170-171 | Migrations must not be gated on anything flakier than the deploy itself; gate on correctness checks | L1 | the project declares the predicate; Tiphys runs it and records the verdict | DR-0029 | Re-dispositioned by M4-P13. The kernel has no migrations (one test fixture). This is a project's CI design rule and the clearest case of a predicate DR-0029 hands to the project. Also failure mode 3 (S8, 236-237) |
| R-072 | S4, 171-172 | CI needs a per-ref concurrency group so superseded runs cancel | L1 | CI config (concurrency group) | M5 | Re-dispositioned by M4-P13. The kernel's own instance shipped in M1-P1 per D-15; what is left is the PRODUCTIZED bootstrap check, which needs a second project to bootstrap, and that is M5. Also adoption step 4 (S9, 259); same artifact as R-097 |
...
| R-097 | S9, 259-260 | CI concurrency group and a migrations/deploys-cannot-diverge check exist before the first merge | L1 | project bootstrap check (doctor) + CI config | M5 | Re-dispositioned by M4-P13. Same CI artifact as R-072; greenfield bootstrap path is M5, which this row's note already said before the bucket agreed with it |
