...
P=$S/CRB9-PRISTINE-3.yaml; cp assurance-modes.yaml $P; PRE=$(md5sum assurance-modes.yaml|cut -d' ' -f1)
...
echo "MUTATE_EXIT=$?"
node dist/bin/tiphys.js validate --type assurance-modes --context . assurance-modes.yaml > $S/CRB9-m3-val.txt 2>&1; echo "VALIDATE_EXIT=$?"; cat $S/CRB9-m3-val.txt
node dist/bin/tiphys.js mode show --mode local-only > $S/CRB9-m3-show.txt 2>&1; echo "SHOW_EXIT=$?"; sed -n '1,3p' $S/CRB9-m3-show.txt
