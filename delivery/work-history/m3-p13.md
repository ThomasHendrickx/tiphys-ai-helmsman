...
--head HEAD --phase m3-p13`, node v26.6.0, `dist/` built. Runner exit 1.
...
Every not-applicable carries its precondition id and evaluation in
`summary.json`; none is a silent skip. Recomputed from the rows: green 9, red 1,
not-applicable 6, error 0. `summary.json` reports declared 16, applicable 10,
verdict 10, green 9, red 1, not-applicable 6, error 0, vacuous 0. The recomputed
counts equal the reported ones. Every green gate has `units` greater than zero.
